...
def cookie_keep_alive():
    ...
    # 為避免解析過於頻繁產生負面影響, 每 8 小時最多執行一次
    global _last_cookie_keep_alive
    if time.time() - _last_cookie_keep_alive < 8 * 3600:
        return
    ...
    try:
        ...
        _last_cookie_keep_alive = time.time()
...
_last_cookie_keep_alive = 0  # 上次 cookie 保活時間戳 (unix time)
